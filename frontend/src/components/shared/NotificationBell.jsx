...
console.log("user",user.role);
  ...
  useEffect(() => {
    ...
    const handleNewNotification = (newNotif) => {
      ...
      fetchNotifications(); // Refresh full list
    };
  ...
  <div className="relative" ref={dropdownRef}>
    ...
    >
      ...
      {unreadCount > 0 && (
        <span className="absolute -top-1 -right-1 flex h-5 min-w-[20px] px-1 items-center justify-center 
      ...
      animate-in fade-in zoom-in-95 duration-200">
        ...
        bg-gradient-to-r from-primary/10 via-transparent to-primary/5">
          ...
          <Button
            variant="ghost"
            size="sm"
            className="text-xs text-primary hover:bg-primary/10 rounded-full"
            onClick={() => {
              setIsOpen(false);
